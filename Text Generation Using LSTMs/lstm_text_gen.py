...
def generate_sequence(model,x,vocabulary_size,idx_dict,seq_out_length=1000):
    """
    Generates output sequences from text model

    Arguments:
    model -- a language model trained on a corpus of text
    x -- list of input training sequences
    vocabulary_size -- number of unique characters in given
                       corpus of text
    idx_dict -- dictionary with integers mapped back to the
                characters they represent                      
    seq_out_length -- default value is 1000. Number of characters
    ...
    for i in range(seq_out_length):
        pattern_temp=np.reshape(pattern,(1,len(pattern),1))
        pattern_temp=pattern_temp / float(vocabulary_size)
        prediction=model.predict(pattern_temp,verbose=0)
        index=np.argmax(prediction)
        result=idx_dict[index]
        sys.stdout.write(result)
        pattern.append(index)
        pattern=pattern[1:len(pattern)]
    print("\nDone.")

#Training corpus is loaded
text,chars,char_to_idx,idx_to_char,n_characters,n_vocabulary=load_data('E:\\aiw.txt')
print('Total number of characters are ',n_characters)
print('Vocabulary size is ',n_vocabulary)

#Training data is created
X,Y,x,y,n_patterns=training_data(text,char_to_idx,100,n_characters,n_vocabulary)
